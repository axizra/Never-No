function showNextQuestion(nextSection, nextQuestion) {
    const currentSectionElement = document.querySelector(".wrapper:not([style='display: none;'])");
    const nextSectionElement = document.getElementById(nextSection);

    if (currentSectionElement && nextSectionElement) {
        currentSectionElement.style.display = 'none';
        nextSectionElement.style.display = 'block';
        question.innerHTML = nextQuestion;
        gif.src = "https://media.giphy.com/media/FTGah7Mx3ss04PcasF/giphy.gif";
    }
}

const question = document.querySelector(".question");
const gif = document.querySelector(".gif");
const yesBtn = document.querySelector(".yes-btn");
const noBtn = document.querySelector(".no-btn");

yesBtn.addEventListener("click", () => {
    showNextQuestion('dateSection', 'Will you go on a date with me?');
});

noBtn.addEventListener("mousemove", moveNoButton);

function moveNoButton() {
    const noBtnRect = noBtn.getBoundingClientRect();
    const maxX = window.innerWidth - noBtnRect.width;
    const maxY = window.innerHeight - noBtnRect.height;

    const randomX = Math.floor(Math.random() * maxX);
    const randomY = Math.floor(Math.random() * maxY);

    noBtn.style.left = randomX + "px";
    noBtn.style.top = randomY + "px";
}
